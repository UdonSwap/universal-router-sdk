...
import { Trade as V2Trade, Route as RouteV2, Pair } from 'udonswap-v2'
import { Trade as V3Trade, Route as RouteV3, Pool } from 'udonswap-v3-sdk'
...
import { buildTrade, getUniswapPools, swapOptions, DAI, ETH, WETH, USDC } from './utils/uniswapData'
import {
  ...
  describe('erc20 --> nft', async () => {
    ...
    it('erc20 -> 1 looksrare nft', async () => {
      const erc20Trade = buildTrade([
        await V3Trade.fromRoute(
          ...
          CurrencyAmount.fromRawAmount(ETH, looksRareV2Value.toString()),
    ...
    it('erc20 -> 1 looksRare nft & 1 seaport nft', async () => {
      ...
      const erc20Trade = buildTrade([
        await V3Trade.fromRoute(
          new RouteV3([WETH_USDC_V3], USDC, ETH),
          CurrencyAmount.fromRawAmount(ETH, totalValue),
          TradeType.EXACT_OUTPUT
        ),
      ])
      const opts = swapOptions({ recipient: ROUTER_AS_RECIPIENT })
    ...
    it('2 erc20s -> 1 NFT', async () => {
      const erc20Trade1 = buildTrade([
        await V3Trade.fromRoute(
          new RouteV3([WETH_USDC_V3], USDC, ETH),
          CurrencyAmount.fromRawAmount(ETH, looksRareV2Value.div(2).toString()),
          TradeType.EXACT_OUTPUT
        ),
      ])
